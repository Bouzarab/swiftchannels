/* ═══════════════════════════════════════════════════════════════════
   Swift Channels — legal page translations
   ───────────────────────────────────────────────────────────────────
   Loaded by legal.html only, after assets/i18n.js, and merged into the
   main table. Keys are the ENGLISH text exactly as it appears on the
   page; anything missing falls back to English.

   These are translations of the English terms, not separate legal
   documents. The English version is the authoritative one.
   ═══════════════════════════════════════════════════════════════════ */
(function(){
  const EXTRA = {

/* ═══════════════════════ FRENCH ═══════════════════════ */
fr: {
  '__legal_title': 'Conditions, remboursements et confidentialité — Swift Channels',
  '__legal_desc': 'Conditions de service, politique de remboursement et notice de confidentialité de Swift Channels.',
  '← Back to the site': '← Retour au site',
  'Terms, refunds and privacy': 'Conditions, remboursements et confidentialité',
  'Last updated 24 August 2026': 'Dernière mise à jour : 24 août 2026',
  'Trading name': 'Nom commercial',
  'Contact': 'Contact',
  '1. Terms of service': '1. Conditions de service',
  '2. Refund policy': '2. Politique de remboursement',
  '3. Privacy notice': '3. Notice de confidentialité',

  'Who we are and what we sell': 'Qui nous sommes et ce que nous vendons',
  'Swift Channels resells access to a streaming television service operated by a third-party platform. We supply you with login credentials for that platform; we do not own, produce, license or control the content it carries.':
    'Swift Channels revend l’accès à un service de télévision en streaming exploité par une plateforme tierce. Nous vous fournissons des identifiants de connexion à cette plateforme ; nous ne possédons, ne produisons, ne concédons sous licence ni ne contrôlons les contenus qu’elle diffuse.',
  'What you are buying': 'Ce que vous achetez',
  'A subscription is access to the platform for a fixed period — 30 days or 12 months — for the number of simultaneous connections you selected. Channel line-ups, on-demand catalogues and programme guides are set by the platform operator and can change at any time without notice. We cannot guarantee that any particular channel, match, film or series will be available for the whole of your subscription.':
    'Un abonnement donne accès à la plateforme pour une durée déterminée — 30 jours ou 12 mois — pour le nombre de connexions simultanées que vous avez choisi. Les grilles de chaînes, les catalogues à la demande et les guides de programmes sont définis par l’opérateur de la plateforme et peuvent changer à tout moment sans préavis. Nous ne pouvons garantir qu’une chaîne, un match, un film ou une série en particulier restera disponible pendant toute la durée de votre abonnement.',
  'Household use only': 'Usage domestique uniquement',
  'Your credentials are for private use in a single household. You may not resell them, share them publicly, use them for commercial or public screening, or redistribute the stream. If we find credentials being used this way, we may close the account without a refund.':
    'Vos identifiants sont destinés à un usage privé au sein d’un seul foyer. Vous ne pouvez pas les revendre, les partager publiquement, les utiliser pour une diffusion commerciale ou publique, ni redistribuer le flux. Si nous constatons un tel usage, nous pouvons fermer le compte sans remboursement.',
  'Your responsibilities': 'Vos responsabilités',
  'Give accurate contact details so we can deliver your credentials.':
    'Donner des coordonnées exactes pour que nous puissions vous livrer vos identifiants.',
  'Provide your own internet connection and a compatible device. Roughly 5 Mbps is needed for SD, 15 Mbps for HD and 25 Mbps for 4K.':
    'Fournir votre propre connexion internet et un appareil compatible. Il faut environ 5 Mbps pour la SD, 15 Mbps pour la HD et 25 Mbps pour la 4K.',
  'Keep your credentials private.': 'Garder vos identifiants confidentiels.',
  'Comply with the laws that apply where you live, including any rules about which broadcast content you may lawfully access. You are responsible for that; we cannot advise on it.':
    'Respecter les lois applicables là où vous résidez, y compris les règles relatives aux contenus audiovisuels auxquels vous pouvez légalement accéder. Cela relève de votre responsabilité ; nous ne pouvons pas vous conseiller sur ce point.',
  'Delivery': 'Livraison',
  'We aim to send credentials within ten minutes during support hours. Orders placed overnight are handled the following morning. Delivery is by WhatsApp or email to the contact details you gave us.':
    'Nous visons un envoi des identifiants en moins de dix minutes pendant les heures de support. Les commandes passées la nuit sont traitées le lendemain matin. La livraison se fait par WhatsApp ou par e-mail, aux coordonnées que vous nous avez données.',
  'Availability and interruptions': 'Disponibilité et interruptions',
  "We target high availability but cannot promise an uninterrupted service. Outages on the platform operator's side, on your internet connection, or on your device are outside our control. Where a line is genuinely down for an extended period, we will replace it and extend your subscription by the time lost. That is the remedy we offer.":
    'Nous visons une disponibilité élevée mais ne pouvons pas promettre un service ininterrompu. Les pannes du côté de l’opérateur de la plateforme, de votre connexion internet ou de votre appareil échappent à notre contrôle. Lorsqu’une ligne est réellement hors service pendant une période prolongée, nous la remplaçons et prolongeons votre abonnement de la durée perdue. C’est la solution que nous proposons.',
  'Free 24-hour test': 'Test gratuit de 24 heures',
  'One free test per household, lasting 24 hours from activation. It is the full service so you can judge it before paying. We may decline a test if we believe the offer is being abused.':
    'Un test gratuit par foyer, d’une durée de 24 heures à partir de l’activation. Il s’agit du service complet, afin que vous puissiez juger avant de payer. Nous pouvons refuser un test si nous estimons que l’offre fait l’objet d’un abus.',
  'Ending a subscription': 'Fin d’un abonnement',
  'Subscriptions do not renew automatically. Yours simply ends on its expiry date unless you message us to extend it. You can stop using the service at any time; see the refund policy below for what that means financially.':
    'Les abonnements ne se renouvellent pas automatiquement. Le vôtre prend simplement fin à sa date d’expiration, sauf si vous nous écrivez pour le prolonger. Vous pouvez cesser d’utiliser le service à tout moment ; voyez la politique de remboursement ci-dessous pour les conséquences financières.',
  'Our liability': 'Notre responsabilité',
  'Our liability to you is limited to the amount you paid for your current subscription period. We are not liable for indirect losses. Nothing here limits liability that cannot lawfully be limited.':
    'Notre responsabilité envers vous est limitée au montant payé pour la période d’abonnement en cours. Nous ne sommes pas responsables des dommages indirects. Rien ici ne limite une responsabilité qui ne peut légalement l’être.',
  'Changes to these terms': 'Modification des présentes conditions',
  'We may update these terms. The version on this page at the time of your order is the one that applies to that order.':
    'Nous pouvons mettre à jour ces conditions. La version figurant sur cette page au moment de votre commande est celle qui s’applique à cette commande.',
  'Law': 'Droit applicable',
  'These terms are governed by Moroccan law. If you are a consumer resident in the EU or UK, this does not deprive you of the protection of mandatory consumer law in your country of residence, and you keep the right to bring proceedings there.':
    'Les présentes conditions sont régies par le droit marocain. Si vous êtes un consommateur résidant dans l’UE ou au Royaume-Uni, cela ne vous prive pas de la protection des dispositions impératives du droit de la consommation de votre pays de résidence, et vous conservez le droit d’agir en justice dans ce pays.',

  'In short': 'En bref',
  'Once your credentials have been sent, the subscription is not refundable.':
    'Une fois vos identifiants envoyés, l’abonnement n’est pas remboursable.',
  'The free 24-hour test exists so you can try the full service on your own device before you pay anything.':
    'Le test gratuit de 24 heures existe précisément pour que vous puissiez essayer le service complet sur votre appareil avant de payer quoi que ce soit.',
  'Why': 'Pourquoi',
  'A subscription is digital content delivered immediately. Once credentials are issued the access has been granted and cannot be returned.':
    'Un abonnement est un contenu numérique fourni immédiatement. Une fois les identifiants émis, l’accès est accordé et ne peut pas être restitué.',
  'Your EU right of withdrawal, and how it applies': 'Votre droit de rétractation dans l’UE, et son application',
  'If you are a consumer in the EU or UK, you normally have 14 days to withdraw from an online purchase. For digital content delivered immediately, that right is lost once delivery begins — but only if you asked for immediate delivery and acknowledged that you would lose the right.':
    'Si vous êtes un consommateur dans l’UE ou au Royaume-Uni, vous disposez normalement de 14 jours pour vous rétracter d’un achat en ligne. Pour un contenu numérique fourni immédiatement, ce droit est perdu dès le début de la fourniture — mais uniquement si vous avez demandé cette fourniture immédiate et reconnu que vous perdiez ce droit.',
  'That is exactly what the confirmation box on our order form does. By ticking it you ask us to supply your credentials straight away and accept that your 14-day withdrawal right ends when we send them.':
    'C’est exactement la fonction de la case de confirmation de notre formulaire de commande. En la cochant, vous nous demandez de vous fournir vos identifiants immédiatement et acceptez que votre droit de rétractation de 14 jours prenne fin lorsque nous les envoyons.',
  'If you have not yet received your credentials, the withdrawal right still stands.':
    'Si vous n’avez pas encore reçu vos identifiants, le droit de rétractation demeure.',
  'Message us before delivery and we will refund you in full.':
    'Écrivez-nous avant la livraison et nous vous rembourserons intégralement.',
  'When we do refund': 'Quand nous remboursons',
  'You paid but we never delivered your credentials.':
    'Vous avez payé mais nous ne vous avons jamais livré vos identifiants.',
  'You were charged twice for the same subscription.':
    'Vous avez été débité deux fois pour le même abonnement.',
  'We are unable to provide the service at all for a sustained period and cannot replace your line.':
    'Nous sommes totalement incapables de fournir le service pendant une période prolongée et ne pouvons pas remplacer votre ligne.',
  'In those cases we refund to the original payment method. Allow up to ten working days for it to appear.':
    'Dans ces cas, nous remboursons sur le moyen de paiement d’origine. Comptez jusqu’à dix jours ouvrables avant que la somme apparaisse.',
  'When we do not refund': 'Quand nous ne remboursons pas',
  'You changed your mind after receiving working credentials.':
    'Vous avez changé d’avis après avoir reçu des identifiants fonctionnels.',
  'A specific channel or event you wanted is not carried, or was removed by the platform.':
    'Une chaîne ou un événement précis que vous vouliez n’est pas proposé, ou a été retiré par la plateforme.',
  'Your internet connection or device cannot handle the stream.':
    'Votre connexion internet ou votre appareil ne peut pas supporter le flux.',
  'Your account was closed for breaching the household-use rule.':
    'Votre compte a été fermé pour non-respect de la règle d’usage domestique.',
  'Problems before you ask for money back': 'Les problèmes avant une demande de remboursement',
  'Most faults are fixable in minutes. Message us on WhatsApp first — replacing a line, resetting credentials or reconfiguring an app resolves the large majority of issues, and we add any lost time to your subscription.':
    'La plupart des pannes se règlent en quelques minutes. Écrivez-nous d’abord sur WhatsApp — remplacer une ligne, réinitialiser des identifiants ou reconfigurer une application résout la grande majorité des problèmes, et nous ajoutons le temps perdu à votre abonnement.',

  'Who controls your data': 'Qui est responsable de vos données',
  'Swift Channels. Contact:': 'Swift Channels. Contact :',
  'Worth knowing': 'Bon à savoir',
  'This website has no database, no accounts and no tracking. The order form runs entirely in your browser and builds a message for you to send us. Nothing is submitted to a server here.':
    'Ce site n’a ni base de données, ni comptes, ni traçage. Le formulaire de commande fonctionne entièrement dans votre navigateur et compose un message que vous nous envoyez vous-même. Rien n’est transmis à un serveur ici.',
  'What we collect and why': 'Ce que nous collectons et pourquoi',
  'What you send us': 'Ce que vous nous envoyez',
  '— your name, WhatsApp number, email if you give one, your device and any MAC address, and your order details. We need these to create your account and deliver your credentials. Legal basis: performance of a contract.':
    '— votre nom, votre numéro WhatsApp, votre e-mail le cas échéant, votre appareil et son adresse MAC éventuelle, ainsi que les détails de votre commande. Ces éléments nous sont nécessaires pour créer votre compte et vous livrer vos identifiants. Base légale : exécution d’un contrat.',
  'Payment records': 'Traces de paiement',
  '— we see the name, amount and date associated with your payment, and the receipt you send us. We never see your card number. Legal basis: contract and legal record-keeping.':
    '— nous voyons le nom, le montant et la date associés à votre paiement, ainsi que le reçu que vous nous envoyez. Nous ne voyons jamais votre numéro de carte. Base légale : contrat et obligations de conservation.',
  'Your messages': 'Vos messages',
  '— the WhatsApp or email conversation, kept so we can support you and handle renewals or disputes.':
    '— la conversation WhatsApp ou e-mail, conservée pour pouvoir vous assister et gérer les renouvellements ou les litiges.',
  'What we do not collect': 'Ce que nous ne collectons pas',
  'No cookies, analytics, advertising pixels or visitor tracking on this site.':
    'Aucun cookie, aucune analyse d’audience, aucun pixel publicitaire, aucun suivi des visiteurs sur ce site.',
  'No card or bank details — payments are handled entirely by PayPal.':
    'Aucune donnée bancaire — les paiements sont entièrement traités par PayPal.',
  'No record of what you watch. We do not have access to your viewing activity.':
    'Aucun relevé de ce que vous regardez. Nous n’avons pas accès à votre activité de visionnage.',
  'Local storage': 'Stockage local',
  'When you click through to pay, your order details are saved in your own browser so the confirmation page can show them back to you. This stays on your device, is never transmitted to us, and you can clear it through your browser at any time.':
    'Lorsque vous poursuivez vers le paiement, les détails de votre commande sont enregistrés dans votre propre navigateur afin que la page de confirmation puisse vous les réafficher. Ces données restent sur votre appareil, ne nous sont jamais transmises, et vous pouvez les effacer depuis votre navigateur à tout moment.',
  'Who we share it with': 'Avec qui nous les partageons',
  'The platform operator': 'L’opérateur de la plateforme',
  '— the minimum needed to create and maintain your account.':
    '— le minimum nécessaire pour créer et maintenir votre compte.',
  'Payment providers': 'Prestataires de paiement',
  '— PayPal, which processes your payment under its own privacy policy.':
    '— PayPal, qui traite votre paiement selon sa propre politique de confidentialité.',
  "— as the channel we communicate through, under Meta's policies.":
    '— en tant que canal de communication, selon les règles de Meta.',
  'We do not sell your data or share it for marketing.':
    'Nous ne vendons pas vos données et ne les partageons pas à des fins de marketing.',
  'International transfers': 'Transferts internationaux',
  'We are based in Morocco, so if you are in the EU or UK your data is processed outside your country. We keep transfers to the minimum needed to run your subscription.':
    'Nous sommes établis au Maroc : si vous êtes dans l’UE ou au Royaume-Uni, vos données sont donc traitées hors de votre pays. Nous limitons les transferts au strict nécessaire pour gérer votre abonnement.',
  'How long we keep it': 'Durée de conservation',
  'Order and payment records for three years after your subscription ends, so we can handle disputes and meet record-keeping obligations. Message threads are kept while you are a customer and for a reasonable period afterwards.':
    'Les traces de commande et de paiement sont conservées trois ans après la fin de votre abonnement, afin de gérer les litiges et de respecter nos obligations de conservation. Les conversations sont conservées tant que vous êtes client et pendant une période raisonnable ensuite.',
  'Your rights': 'Vos droits',
  'If you are in the EU or UK you can ask us to give you a copy of your data, correct it, delete it, restrict how we use it, or object to our using it. Email':
    'Si vous êtes dans l’UE ou au Royaume-Uni, vous pouvez nous demander une copie de vos données, leur rectification, leur effacement, la limitation de leur utilisation, ou vous opposer à celle-ci. Écrivez à',
  'and we will respond within 30 days. If you are unhappy with our response you can complain to the data protection authority in your country.':
    'et nous répondrons sous 30 jours. Si notre réponse ne vous satisfait pas, vous pouvez saisir l’autorité de protection des données de votre pays.',
  'Children': 'Mineurs',
  'This service is not intended for anyone under 18 and we do not knowingly collect data from children.':
    'Ce service n’est pas destiné aux personnes de moins de 18 ans et nous ne collectons pas sciemment de données concernant des enfants.',
  'Changes': 'Modifications',
  'If we change this notice we will update the date at the top of this page.':
    'Si nous modifions cette notice, nous mettrons à jour la date en haut de cette page.'
},

/* ═══════════════════════ SPANISH ═══════════════════════ */
es: {
  '__legal_title': 'Términos, reembolsos y privacidad — Swift Channels',
  '__legal_desc': 'Términos del servicio, política de reembolso y aviso de privacidad de Swift Channels.',
  '← Back to the site': '← Volver al sitio',
  'Terms, refunds and privacy': 'Términos, reembolsos y privacidad',
  'Last updated 24 August 2026': 'Última actualización: 24 de agosto de 2026',
  'Trading name': 'Nombre comercial',
  'Contact': 'Contacto',
  '1. Terms of service': '1. Términos del servicio',
  '2. Refund policy': '2. Política de reembolso',
  '3. Privacy notice': '3. Aviso de privacidad',

  'Who we are and what we sell': 'Quiénes somos y qué vendemos',
  'Swift Channels resells access to a streaming television service operated by a third-party platform. We supply you with login credentials for that platform; we do not own, produce, license or control the content it carries.':
    'Swift Channels revende el acceso a un servicio de televisión en streaming operado por una plataforma de terceros. Te facilitamos los datos de acceso a esa plataforma; no somos propietarios, ni producimos, ni licenciamos ni controlamos los contenidos que ofrece.',
  'What you are buying': 'Qué estás contratando',
  'A subscription is access to the platform for a fixed period — 30 days or 12 months — for the number of simultaneous connections you selected. Channel line-ups, on-demand catalogues and programme guides are set by the platform operator and can change at any time without notice. We cannot guarantee that any particular channel, match, film or series will be available for the whole of your subscription.':
    'Una suscripción es el acceso a la plataforma durante un periodo determinado — 30 días o 12 meses — para el número de conexiones simultáneas que elegiste. Las listas de canales, los catálogos a la carta y las guías de programación las fija el operador de la plataforma y pueden cambiar en cualquier momento sin aviso. No podemos garantizar que un canal, partido, película o serie concreta esté disponible durante toda tu suscripción.',
  'Household use only': 'Solo para uso doméstico',
  'Your credentials are for private use in a single household. You may not resell them, share them publicly, use them for commercial or public screening, or redistribute the stream. If we find credentials being used this way, we may close the account without a refund.':
    'Tus datos de acceso son para uso privado en un solo hogar. No puedes revenderlos, compartirlos públicamente, usarlos para exhibición comercial o pública, ni redistribuir la emisión. Si detectamos un uso así, podemos cerrar la cuenta sin reembolso.',
  'Your responsibilities': 'Tus responsabilidades',
  'Give accurate contact details so we can deliver your credentials.':
    'Facilitar datos de contacto correctos para que podamos entregarte los accesos.',
  'Provide your own internet connection and a compatible device. Roughly 5 Mbps is needed for SD, 15 Mbps for HD and 25 Mbps for 4K.':
    'Aportar tu propia conexión a internet y un dispositivo compatible. Hacen falta unos 5 Mbps para SD, 15 Mbps para HD y 25 Mbps para 4K.',
  'Keep your credentials private.': 'Mantener tus datos de acceso en privado.',
  'Comply with the laws that apply where you live, including any rules about which broadcast content you may lawfully access. You are responsible for that; we cannot advise on it.':
    'Cumplir las leyes de tu lugar de residencia, incluidas las normas sobre qué contenidos audiovisuales puedes ver legalmente. Eso es responsabilidad tuya; no podemos asesorarte al respecto.',
  'Delivery': 'Entrega',
  'We aim to send credentials within ten minutes during support hours. Orders placed overnight are handled the following morning. Delivery is by WhatsApp or email to the contact details you gave us.':
    'Procuramos enviar los accesos en menos de diez minutos dentro del horario de soporte. Los pedidos hechos de madrugada se atienden a la mañana siguiente. La entrega se hace por WhatsApp o correo a los datos que nos diste.',
  'Availability and interruptions': 'Disponibilidad e interrupciones',
  "We target high availability but cannot promise an uninterrupted service. Outages on the platform operator's side, on your internet connection, or on your device are outside our control. Where a line is genuinely down for an extended period, we will replace it and extend your subscription by the time lost. That is the remedy we offer.":
    'Aspiramos a una alta disponibilidad, pero no podemos prometer un servicio ininterrumpido. Las caídas del lado del operador de la plataforma, de tu conexión o de tu dispositivo quedan fuera de nuestro control. Cuando una línea está realmente caída durante un periodo prolongado, la sustituimos y ampliamos tu suscripción por el tiempo perdido. Esa es la solución que ofrecemos.',
  'Free 24-hour test': 'Prueba gratuita de 24 horas',
  'One free test per household, lasting 24 hours from activation. It is the full service so you can judge it before paying. We may decline a test if we believe the offer is being abused.':
    'Una prueba gratuita por hogar, de 24 horas desde la activación. Es el servicio completo para que puedas juzgarlo antes de pagar. Podemos denegar una prueba si creemos que se está abusando de la oferta.',
  'Ending a subscription': 'Fin de una suscripción',
  'Subscriptions do not renew automatically. Yours simply ends on its expiry date unless you message us to extend it. You can stop using the service at any time; see the refund policy below for what that means financially.':
    'Las suscripciones no se renuevan automáticamente. La tuya simplemente termina en su fecha de vencimiento salvo que nos escribas para ampliarla. Puedes dejar de usar el servicio cuando quieras; consulta abajo la política de reembolso para saber qué implica económicamente.',
  'Our liability': 'Nuestra responsabilidad',
  'Our liability to you is limited to the amount you paid for your current subscription period. We are not liable for indirect losses. Nothing here limits liability that cannot lawfully be limited.':
    'Nuestra responsabilidad frente a ti se limita al importe que pagaste por el periodo de suscripción en curso. No respondemos de daños indirectos. Nada de lo aquí escrito limita una responsabilidad que legalmente no pueda limitarse.',
  'Changes to these terms': 'Cambios en estos términos',
  'We may update these terms. The version on this page at the time of your order is the one that applies to that order.':
    'Podemos actualizar estos términos. La versión publicada en esta página en el momento de tu pedido es la que se aplica a ese pedido.',
  'Law': 'Ley aplicable',
  'These terms are governed by Moroccan law. If you are a consumer resident in the EU or UK, this does not deprive you of the protection of mandatory consumer law in your country of residence, and you keep the right to bring proceedings there.':
    'Estos términos se rigen por la ley marroquí. Si eres consumidor residente en la UE o el Reino Unido, esto no te priva de la protección de las normas imperativas de consumo de tu país de residencia, y conservas el derecho a reclamar allí.',

  'In short': 'En resumen',
  'Once your credentials have been sent, the subscription is not refundable.':
    'Una vez enviados tus datos de acceso, la suscripción no es reembolsable.',
  'The free 24-hour test exists so you can try the full service on your own device before you pay anything.':
    'La prueba gratuita de 24 horas existe para que puedas probar el servicio completo en tu propio dispositivo antes de pagar nada.',
  'Why': 'Por qué',
  'A subscription is digital content delivered immediately. Once credentials are issued the access has been granted and cannot be returned.':
    'Una suscripción es contenido digital entregado de inmediato. Una vez emitidos los accesos, el acceso ya está concedido y no puede devolverse.',
  'Your EU right of withdrawal, and how it applies': 'Tu derecho de desistimiento en la UE y cómo se aplica',
  'If you are a consumer in the EU or UK, you normally have 14 days to withdraw from an online purchase. For digital content delivered immediately, that right is lost once delivery begins — but only if you asked for immediate delivery and acknowledged that you would lose the right.':
    'Si eres consumidor en la UE o el Reino Unido, normalmente tienes 14 días para desistir de una compra online. En el contenido digital entregado de inmediato, ese derecho se pierde en cuanto empieza la entrega — pero solo si pediste la entrega inmediata y reconociste que perderías el derecho.',
  'That is exactly what the confirmation box on our order form does. By ticking it you ask us to supply your credentials straight away and accept that your 14-day withdrawal right ends when we send them.':
    'Eso es exactamente lo que hace la casilla de confirmación de nuestro formulario. Al marcarla nos pides que te enviemos los accesos de inmediato y aceptas que tu derecho de desistimiento de 14 días termina cuando te los enviamos.',
  'If you have not yet received your credentials, the withdrawal right still stands.':
    'Si todavía no has recibido tus datos de acceso, el derecho de desistimiento sigue vigente.',
  'Message us before delivery and we will refund you in full.':
    'Escríbenos antes de la entrega y te reembolsaremos íntegramente.',
  'When we do refund': 'Cuándo sí reembolsamos',
  'You paid but we never delivered your credentials.':
    'Pagaste pero nunca te entregamos los datos de acceso.',
  'You were charged twice for the same subscription.':
    'Se te cobró dos veces la misma suscripción.',
  'We are unable to provide the service at all for a sustained period and cannot replace your line.':
    'No podemos prestar el servicio en absoluto durante un periodo prolongado y no podemos sustituir tu línea.',
  'In those cases we refund to the original payment method. Allow up to ten working days for it to appear.':
    'En esos casos reembolsamos al medio de pago original. Puede tardar hasta diez días hábiles en aparecer.',
  'When we do not refund': 'Cuándo no reembolsamos',
  'You changed your mind after receiving working credentials.':
    'Cambiaste de opinión después de recibir accesos que funcionaban.',
  'A specific channel or event you wanted is not carried, or was removed by the platform.':
    'Un canal o evento concreto que querías no está incluido, o la plataforma lo retiró.',
  'Your internet connection or device cannot handle the stream.':
    'Tu conexión o tu dispositivo no pueden con la emisión.',
  'Your account was closed for breaching the household-use rule.':
    'Tu cuenta se cerró por incumplir la norma de uso doméstico.',
  'Problems before you ask for money back': 'Problemas antes de pedir el dinero',
  'Most faults are fixable in minutes. Message us on WhatsApp first — replacing a line, resetting credentials or reconfiguring an app resolves the large majority of issues, and we add any lost time to your subscription.':
    'La mayoría de fallos se arreglan en minutos. Escríbenos primero por WhatsApp — sustituir una línea, restablecer los accesos o reconfigurar una app resuelve la gran mayoría de incidencias, y añadimos el tiempo perdido a tu suscripción.',

  'Who controls your data': 'Quién controla tus datos',
  'Swift Channels. Contact:': 'Swift Channels. Contacto:',
  'Worth knowing': 'Conviene saberlo',
  'This website has no database, no accounts and no tracking. The order form runs entirely in your browser and builds a message for you to send us. Nothing is submitted to a server here.':
    'Esta web no tiene base de datos, ni cuentas, ni seguimiento. El formulario funciona enteramente en tu navegador y compone un mensaje que nos envías tú. Aquí no se envía nada a ningún servidor.',
  'What we collect and why': 'Qué recogemos y por qué',
  'What you send us': 'Lo que nos envías',
  '— your name, WhatsApp number, email if you give one, your device and any MAC address, and your order details. We need these to create your account and deliver your credentials. Legal basis: performance of a contract.':
    '— tu nombre, tu número de WhatsApp, tu correo si lo das, tu dispositivo y su dirección MAC si procede, y los datos de tu pedido. Los necesitamos para crear tu cuenta y entregarte los accesos. Base jurídica: ejecución de un contrato.',
  'Payment records': 'Registros de pago',
  '— we see the name, amount and date associated with your payment, and the receipt you send us. We never see your card number. Legal basis: contract and legal record-keeping.':
    '— vemos el nombre, el importe y la fecha asociados a tu pago, y el recibo que nos envías. Nunca vemos el número de tu tarjeta. Base jurídica: contrato y obligaciones de conservación.',
  'Your messages': 'Tus mensajes',
  '— the WhatsApp or email conversation, kept so we can support you and handle renewals or disputes.':
    '— la conversación de WhatsApp o correo, guardada para poder atenderte y gestionar renovaciones o reclamaciones.',
  'What we do not collect': 'Qué no recogemos',
  'No cookies, analytics, advertising pixels or visitor tracking on this site.':
    'Ni cookies, ni analítica, ni píxeles publicitarios, ni seguimiento de visitantes en este sitio.',
  'No card or bank details — payments are handled entirely by PayPal.':
    'Ningún dato bancario — los pagos los gestiona íntegramente PayPal.',
  'No record of what you watch. We do not have access to your viewing activity.':
    'Ningún registro de lo que ves. No tenemos acceso a tu actividad de visionado.',
  'Local storage': 'Almacenamiento local',
  'When you click through to pay, your order details are saved in your own browser so the confirmation page can show them back to you. This stays on your device, is never transmitted to us, and you can clear it through your browser at any time.':
    'Cuando continúas para pagar, los datos de tu pedido se guardan en tu propio navegador para que la página de confirmación pueda mostrártelos. Se quedan en tu dispositivo, nunca se nos transmiten y puedes borrarlos desde tu navegador cuando quieras.',
  'Who we share it with': 'Con quién los compartimos',
  'The platform operator': 'El operador de la plataforma',
  '— the minimum needed to create and maintain your account.':
    '— lo mínimo necesario para crear y mantener tu cuenta.',
  'Payment providers': 'Proveedores de pago',
  '— PayPal, which processes your payment under its own privacy policy.':
    '— PayPal, que procesa tu pago según su propia política de privacidad.',
  "— as the channel we communicate through, under Meta's policies.":
    '— como canal por el que nos comunicamos, según las políticas de Meta.',
  'We do not sell your data or share it for marketing.':
    'No vendemos tus datos ni los compartimos con fines publicitarios.',
  'International transfers': 'Transferencias internacionales',
  'We are based in Morocco, so if you are in the EU or UK your data is processed outside your country. We keep transfers to the minimum needed to run your subscription.':
    'Estamos radicados en Marruecos, así que si estás en la UE o el Reino Unido tus datos se tratan fuera de tu país. Limitamos las transferencias al mínimo necesario para gestionar tu suscripción.',
  'How long we keep it': 'Cuánto tiempo los conservamos',
  'Order and payment records for three years after your subscription ends, so we can handle disputes and meet record-keeping obligations. Message threads are kept while you are a customer and for a reasonable period afterwards.':
    'Los registros de pedido y pago, tres años tras el fin de tu suscripción, para poder atender reclamaciones y cumplir obligaciones de conservación. Las conversaciones se guardan mientras eres cliente y durante un periodo razonable después.',
  'Your rights': 'Tus derechos',
  'If you are in the EU or UK you can ask us to give you a copy of your data, correct it, delete it, restrict how we use it, or object to our using it. Email':
    'Si estás en la UE o el Reino Unido puedes pedirnos una copia de tus datos, su rectificación, su supresión, la limitación de su uso, u oponerte a él. Escribe a',
  'and we will respond within 30 days. If you are unhappy with our response you can complain to the data protection authority in your country.':
    'y responderemos en un plazo de 30 días. Si nuestra respuesta no te satisface, puedes reclamar ante la autoridad de protección de datos de tu país.',
  'Children': 'Menores',
  'This service is not intended for anyone under 18 and we do not knowingly collect data from children.':
    'Este servicio no está dirigido a menores de 18 años y no recogemos conscientemente datos de menores.',
  'Changes': 'Cambios',
  'If we change this notice we will update the date at the top of this page.':
    'Si modificamos este aviso, actualizaremos la fecha en la parte superior de esta página.'
},

/* ═══════════════════════ GERMAN ═══════════════════════ */
de: {
  '__legal_title': 'AGB, Erstattungen und Datenschutz — Swift Channels',
  '__legal_desc': 'Allgemeine Geschäftsbedingungen, Erstattungsrichtlinie und Datenschutzhinweise von Swift Channels.',
  '← Back to the site': '← Zurück zur Website',
  'Terms, refunds and privacy': 'AGB, Erstattungen und Datenschutz',
  'Last updated 24 August 2026': 'Zuletzt aktualisiert: 24. August 2026',
  'Trading name': 'Handelsname',
  'Contact': 'Kontakt',
  '1. Terms of service': '1. Allgemeine Geschäftsbedingungen',
  '2. Refund policy': '2. Erstattungsrichtlinie',
  '3. Privacy notice': '3. Datenschutzhinweise',

  'Who we are and what we sell': 'Wer wir sind und was wir verkaufen',
  'Swift Channels resells access to a streaming television service operated by a third-party platform. We supply you with login credentials for that platform; we do not own, produce, license or control the content it carries.':
    'Swift Channels vertreibt den Zugang zu einem Streaming-Fernsehdienst, der von einer Plattform eines Drittanbieters betrieben wird. Wir stellen Ihnen Zugangsdaten für diese Plattform bereit; wir besitzen, produzieren, lizenzieren und kontrollieren die dort verfügbaren Inhalte nicht.',
  'What you are buying': 'Was Sie erwerben',
  'A subscription is access to the platform for a fixed period — 30 days or 12 months — for the number of simultaneous connections you selected. Channel line-ups, on-demand catalogues and programme guides are set by the platform operator and can change at any time without notice. We cannot guarantee that any particular channel, match, film or series will be available for the whole of your subscription.':
    'Ein Abonnement ist der Zugang zur Plattform für einen festen Zeitraum — 30 Tage oder 12 Monate — für die von Ihnen gewählte Zahl gleichzeitiger Verbindungen. Senderlisten, Abrufkataloge und Programmführer werden vom Plattformbetreiber festgelegt und können sich jederzeit ohne Ankündigung ändern. Wir können nicht garantieren, dass ein bestimmter Sender, ein Spiel, ein Film oder eine Serie während der gesamten Laufzeit verfügbar bleibt.',
  'Household use only': 'Nur für den Hausgebrauch',
  'Your credentials are for private use in a single household. You may not resell them, share them publicly, use them for commercial or public screening, or redistribute the stream. If we find credentials being used this way, we may close the account without a refund.':
    'Ihre Zugangsdaten sind für die private Nutzung in einem einzigen Haushalt bestimmt. Sie dürfen sie nicht weiterverkaufen, öffentlich teilen, gewerblich oder öffentlich vorführen oder den Stream weiterverbreiten. Stellen wir eine solche Nutzung fest, können wir das Konto ohne Erstattung schließen.',
  'Your responsibilities': 'Ihre Pflichten',
  'Give accurate contact details so we can deliver your credentials.':
    'Korrekte Kontaktdaten angeben, damit wir Ihre Zugangsdaten zustellen können.',
  'Provide your own internet connection and a compatible device. Roughly 5 Mbps is needed for SD, 15 Mbps for HD and 25 Mbps for 4K.':
    'Für eigene Internetverbindung und ein kompatibles Gerät sorgen. Nötig sind etwa 5 Mbit/s für SD, 15 Mbit/s für HD und 25 Mbit/s für 4K.',
  'Keep your credentials private.': 'Ihre Zugangsdaten geheim halten.',
  'Comply with the laws that apply where you live, including any rules about which broadcast content you may lawfully access. You are responsible for that; we cannot advise on it.':
    'Die an Ihrem Wohnort geltenden Gesetze einhalten, einschließlich der Regeln dazu, welche Rundfunkinhalte Sie rechtmäßig nutzen dürfen. Dafür sind Sie verantwortlich; wir können dazu nicht beraten.',
  'Delivery': 'Lieferung',
  'We aim to send credentials within ten minutes during support hours. Orders placed overnight are handled the following morning. Delivery is by WhatsApp or email to the contact details you gave us.':
    'Wir streben an, Zugangsdaten innerhalb von zehn Minuten während der Supportzeiten zu versenden. Nachts aufgegebene Bestellungen werden am folgenden Morgen bearbeitet. Die Lieferung erfolgt per WhatsApp oder E-Mail an die von Ihnen angegebenen Kontaktdaten.',
  'Availability and interruptions': 'Verfügbarkeit und Störungen',
  "We target high availability but cannot promise an uninterrupted service. Outages on the platform operator's side, on your internet connection, or on your device are outside our control. Where a line is genuinely down for an extended period, we will replace it and extend your subscription by the time lost. That is the remedy we offer.":
    'Wir streben hohe Verfügbarkeit an, können einen unterbrechungsfreien Dienst aber nicht zusichern. Störungen beim Plattformbetreiber, an Ihrer Internetverbindung oder an Ihrem Gerät liegen außerhalb unseres Einflusses. Fällt eine Leitung tatsächlich über längere Zeit aus, ersetzen wir sie und verlängern Ihr Abonnement um die verlorene Zeit. Das ist die Abhilfe, die wir anbieten.',
  'Free 24-hour test': 'Kostenloser 24-Stunden-Test',
  'One free test per household, lasting 24 hours from activation. It is the full service so you can judge it before paying. We may decline a test if we believe the offer is being abused.':
    'Ein kostenloser Test pro Haushalt, 24 Stunden ab Freischaltung. Es ist der volle Dienst, damit Sie ihn vor dem Bezahlen beurteilen können. Wir können einen Test ablehnen, wenn wir einen Missbrauch des Angebots vermuten.',
  'Ending a subscription': 'Beendigung eines Abonnements',
  'Subscriptions do not renew automatically. Yours simply ends on its expiry date unless you message us to extend it. You can stop using the service at any time; see the refund policy below for what that means financially.':
    'Abonnements verlängern sich nicht automatisch. Ihres endet einfach am Ablaufdatum, sofern Sie uns nicht zur Verlängerung schreiben. Sie können die Nutzung jederzeit einstellen; was das finanziell bedeutet, steht in der Erstattungsrichtlinie unten.',
  'Our liability': 'Unsere Haftung',
  'Our liability to you is limited to the amount you paid for your current subscription period. We are not liable for indirect losses. Nothing here limits liability that cannot lawfully be limited.':
    'Unsere Haftung Ihnen gegenüber ist auf den Betrag begrenzt, den Sie für die laufende Abo-Periode gezahlt haben. Für mittelbare Schäden haften wir nicht. Nichts hiervon beschränkt eine Haftung, die gesetzlich nicht beschränkt werden darf.',
  'Changes to these terms': 'Änderungen dieser Bedingungen',
  'We may update these terms. The version on this page at the time of your order is the one that applies to that order.':
    'Wir können diese Bedingungen aktualisieren. Maßgeblich für Ihre Bestellung ist die Fassung, die zum Zeitpunkt der Bestellung auf dieser Seite stand.',
  'Law': 'Anwendbares Recht',
  'These terms are governed by Moroccan law. If you are a consumer resident in the EU or UK, this does not deprive you of the protection of mandatory consumer law in your country of residence, and you keep the right to bring proceedings there.':
    'Diese Bedingungen unterliegen marokkanischem Recht. Sind Sie Verbraucher mit Wohnsitz in der EU oder im Vereinigten Königreich, entzieht Ihnen das nicht den Schutz zwingender Verbrauchervorschriften Ihres Wohnsitzstaates, und Sie behalten das Recht, dort Klage zu erheben.',

  'In short': 'Kurz gesagt',
  'Once your credentials have been sent, the subscription is not refundable.':
    'Sobald Ihre Zugangsdaten versendet wurden, ist das Abonnement nicht erstattungsfähig.',
  'The free 24-hour test exists so you can try the full service on your own device before you pay anything.':
    'Der kostenlose 24-Stunden-Test besteht genau dafür, dass Sie den vollen Dienst auf Ihrem eigenen Gerät ausprobieren, bevor Sie etwas zahlen.',
  'Why': 'Warum',
  'A subscription is digital content delivered immediately. Once credentials are issued the access has been granted and cannot be returned.':
    'Ein Abonnement ist sofort geliefteter digitaler Inhalt. Sind die Zugangsdaten erst ausgestellt, ist der Zugang gewährt und kann nicht zurückgegeben werden.',
  'Your EU right of withdrawal, and how it applies': 'Ihr EU-Widerrufsrecht und wie es hier greift',
  'If you are a consumer in the EU or UK, you normally have 14 days to withdraw from an online purchase. For digital content delivered immediately, that right is lost once delivery begins — but only if you asked for immediate delivery and acknowledged that you would lose the right.':
    'Als Verbraucher in der EU oder im Vereinigten Königreich haben Sie normalerweise 14 Tage, um einen Online-Kauf zu widerrufen. Bei sofort geliefertem digitalem Inhalt erlischt dieses Recht mit Beginn der Lieferung — aber nur, wenn Sie die sofortige Lieferung verlangt und bestätigt haben, dass Sie dadurch das Recht verlieren.',
  'That is exactly what the confirmation box on our order form does. By ticking it you ask us to supply your credentials straight away and accept that your 14-day withdrawal right ends when we send them.':
    'Genau das leistet das Bestätigungskästchen in unserem Bestellformular. Mit dem Ankreuzen verlangen Sie die sofortige Übermittlung Ihrer Zugangsdaten und akzeptieren, dass Ihr 14-tägiges Widerrufsrecht mit dem Versand endet.',
  'If you have not yet received your credentials, the withdrawal right still stands.':
    'Solange Sie Ihre Zugangsdaten noch nicht erhalten haben, besteht das Widerrufsrecht fort.',
  'Message us before delivery and we will refund you in full.':
    'Schreiben Sie uns vor der Lieferung, und wir erstatten Ihnen den vollen Betrag.',
  'When we do refund': 'Wann wir erstatten',
  'You paid but we never delivered your credentials.':
    'Sie haben gezahlt, aber wir haben Ihre Zugangsdaten nie geliefert.',
  'You were charged twice for the same subscription.':
    'Ihnen wurde dasselbe Abonnement zweimal berechnet.',
  'We are unable to provide the service at all for a sustained period and cannot replace your line.':
    'Wir können den Dienst über einen längeren Zeitraum überhaupt nicht erbringen und Ihre Leitung nicht ersetzen.',
  'In those cases we refund to the original payment method. Allow up to ten working days for it to appear.':
    'In diesen Fällen erstatten wir auf das ursprüngliche Zahlungsmittel. Bis zur Gutschrift können bis zu zehn Werktage vergehen.',
  'When we do not refund': 'Wann wir nicht erstatten',
  'You changed your mind after receiving working credentials.':
    'Sie haben es sich anders überlegt, nachdem Sie funktionierende Zugangsdaten erhalten haben.',
  'A specific channel or event you wanted is not carried, or was removed by the platform.':
    'Ein bestimmter Sender oder ein Ereignis, das Sie wollten, wird nicht geführt oder wurde von der Plattform entfernt.',
  'Your internet connection or device cannot handle the stream.':
    'Ihre Internetverbindung oder Ihr Gerät kommt mit dem Stream nicht zurecht.',
  'Your account was closed for breaching the household-use rule.':
    'Ihr Konto wurde wegen Verstoßes gegen die Haushaltsregel geschlossen.',
  'Problems before you ask for money back': 'Probleme, bevor Sie Geld zurückfordern',
  'Most faults are fixable in minutes. Message us on WhatsApp first — replacing a line, resetting credentials or reconfiguring an app resolves the large majority of issues, and we add any lost time to your subscription.':
    'Die meisten Störungen sind in Minuten behoben. Schreiben Sie uns zuerst per WhatsApp — eine neue Leitung, zurückgesetzte Zugangsdaten oder eine neu eingerichtete App lösen die große Mehrheit der Fälle, und verlorene Zeit schreiben wir Ihrem Abo gut.',

  'Who controls your data': 'Wer über Ihre Daten entscheidet',
  'Swift Channels. Contact:': 'Swift Channels. Kontakt:',
  'Worth knowing': 'Gut zu wissen',
  'This website has no database, no accounts and no tracking. The order form runs entirely in your browser and builds a message for you to send us. Nothing is submitted to a server here.':
    'Diese Website hat keine Datenbank, keine Konten und kein Tracking. Das Bestellformular läuft vollständig in Ihrem Browser und erstellt eine Nachricht, die Sie uns selbst senden. Hier wird nichts an einen Server übermittelt.',
  'What we collect and why': 'Was wir erheben und warum',
  'What you send us': 'Was Sie uns senden',
  '— your name, WhatsApp number, email if you give one, your device and any MAC address, and your order details. We need these to create your account and deliver your credentials. Legal basis: performance of a contract.':
    '— Ihren Namen, Ihre WhatsApp-Nummer, ggf. Ihre E-Mail, Ihr Gerät und eine etwaige MAC-Adresse sowie Ihre Bestelldaten. Wir brauchen sie, um Ihr Konto anzulegen und die Zugangsdaten zu liefern. Rechtsgrundlage: Vertragserfüllung.',
  'Payment records': 'Zahlungsunterlagen',
  '— we see the name, amount and date associated with your payment, and the receipt you send us. We never see your card number. Legal basis: contract and legal record-keeping.':
    '— wir sehen Name, Betrag und Datum Ihrer Zahlung sowie den Beleg, den Sie uns senden. Ihre Kartennummer sehen wir nie. Rechtsgrundlage: Vertrag und gesetzliche Aufbewahrungspflichten.',
  'Your messages': 'Ihre Nachrichten',
  '— the WhatsApp or email conversation, kept so we can support you and handle renewals or disputes.':
    '— den WhatsApp- oder E-Mail-Verlauf, aufbewahrt, damit wir Sie betreuen und Verlängerungen oder Streitfälle bearbeiten können.',
  'What we do not collect': 'Was wir nicht erheben',
  'No cookies, analytics, advertising pixels or visitor tracking on this site.':
    'Keine Cookies, keine Analyse, keine Werbe-Pixel, kein Besucher-Tracking auf dieser Seite.',
  'No card or bank details — payments are handled entirely by PayPal.':
    'Keine Karten- oder Bankdaten — Zahlungen wickelt vollständig PayPal ab.',
  'No record of what you watch. We do not have access to your viewing activity.':
    'Keine Aufzeichnung dessen, was Sie sehen. Auf Ihr Sehverhalten haben wir keinen Zugriff.',
  'Local storage': 'Lokaler Speicher',
  'When you click through to pay, your order details are saved in your own browser so the confirmation page can show them back to you. This stays on your device, is never transmitted to us, and you can clear it through your browser at any time.':
    'Wenn Sie zur Zahlung weitergehen, werden Ihre Bestelldaten in Ihrem eigenen Browser gespeichert, damit die Bestätigungsseite sie Ihnen anzeigen kann. Sie bleiben auf Ihrem Gerät, werden nie an uns übermittelt, und Sie können sie jederzeit über Ihren Browser löschen.',
  'Who we share it with': 'An wen wir sie weitergeben',
  'The platform operator': 'Den Plattformbetreiber',
  '— the minimum needed to create and maintain your account.':
    '— das Minimum, das zum Anlegen und Führen Ihres Kontos nötig ist.',
  'Payment providers': 'Zahlungsdienstleister',
  '— PayPal, which processes your payment under its own privacy policy.':
    '— PayPal, das Ihre Zahlung nach seiner eigenen Datenschutzerklärung verarbeitet.',
  "— as the channel we communicate through, under Meta's policies.":
    '— als Kanal, über den wir kommunizieren, nach den Richtlinien von Meta.',
  'We do not sell your data or share it for marketing.':
    'Wir verkaufen Ihre Daten nicht und geben sie nicht für Werbezwecke weiter.',
  'International transfers': 'Internationale Übermittlungen',
  'We are based in Morocco, so if you are in the EU or UK your data is processed outside your country. We keep transfers to the minimum needed to run your subscription.':
    'Wir sitzen in Marokko; wenn Sie in der EU oder im Vereinigten Königreich sind, werden Ihre Daten daher außerhalb Ihres Landes verarbeitet. Wir beschränken Übermittlungen auf das für Ihr Abonnement nötige Minimum.',
  'How long we keep it': 'Wie lange wir sie aufbewahren',
  'Order and payment records for three years after your subscription ends, so we can handle disputes and meet record-keeping obligations. Message threads are kept while you are a customer and for a reasonable period afterwards.':
    'Bestell- und Zahlungsunterlagen drei Jahre nach Ende Ihres Abonnements, damit wir Streitfälle bearbeiten und Aufbewahrungspflichten erfüllen können. Nachrichtenverläufe bewahren wir auf, solange Sie Kunde sind, und für eine angemessene Zeit danach.',
  'Your rights': 'Ihre Rechte',
  'If you are in the EU or UK you can ask us to give you a copy of your data, correct it, delete it, restrict how we use it, or object to our using it. Email':
    'In der EU und im Vereinigten Königreich können Sie eine Kopie Ihrer Daten, deren Berichtigung oder Löschung, die Einschränkung der Verarbeitung oder Widerspruch dagegen verlangen. Schreiben Sie an',
  'and we will respond within 30 days. If you are unhappy with our response you can complain to the data protection authority in your country.':
    'und wir antworten innerhalb von 30 Tagen. Sind Sie mit unserer Antwort unzufrieden, können Sie sich bei der Datenschutzbehörde Ihres Landes beschweren.',
  'Children': 'Kinder',
  'This service is not intended for anyone under 18 and we do not knowingly collect data from children.':
    'Dieser Dienst richtet sich nicht an Personen unter 18 Jahren, und wir erheben wissentlich keine Daten von Kindern.',
  'Changes': 'Änderungen',
  'If we change this notice we will update the date at the top of this page.':
    'Wenn wir diese Hinweise ändern, aktualisieren wir das Datum oben auf dieser Seite.'
},

/* ═══════════════════════ ARABIC ═══════════════════════ */
ar: {
  '__legal_title': 'الشروط والاسترجاع والخصوصية — سويفت تشانلز',
  '__legal_desc': 'شروط الخدمة وسياسة الاسترجاع وإشعار الخصوصية لسويفت تشانلز.',
  '← Back to the site': '← العودة إلى الموقع',
  'Terms, refunds and privacy': 'الشروط والاسترجاع والخصوصية',
  'Last updated 24 August 2026': 'آخر تحديث: 24 غشت 2026',
  'Trading name': 'الاسم التجاري',
  'Contact': 'التواصل',
  '1. Terms of service': '1. شروط الخدمة',
  '2. Refund policy': '2. سياسة الاسترجاع',
  '3. Privacy notice': '3. إشعار الخصوصية',

  'Who we are and what we sell': 'من نحن وما الذي نبيعه',
  'Swift Channels resells access to a streaming television service operated by a third-party platform. We supply you with login credentials for that platform; we do not own, produce, license or control the content it carries.':
    'تعيد سويفت تشانلز بيع الوصول إلى خدمة تلفزيون بالبث تديرها منصة تابعة لطرف ثالث. نحن نزوّدك ببيانات الدخول إلى تلك المنصة؛ ولا نملك المحتوى الذي تبثه ولا ننتجه ولا نرخّصه ولا نتحكم فيه.',
  'What you are buying': 'ما الذي تشتريه',
  'A subscription is access to the platform for a fixed period — 30 days or 12 months — for the number of simultaneous connections you selected. Channel line-ups, on-demand catalogues and programme guides are set by the platform operator and can change at any time without notice. We cannot guarantee that any particular channel, match, film or series will be available for the whole of your subscription.':
    'الاشتراك هو وصول إلى المنصة لمدة محددة — 30 يومًا أو 12 شهرًا — بعدد الاتصالات المتزامنة الذي اخترته. قوائم القنوات وكتالوجات المحتوى عند الطلب وأدلة البرامج يحددها مشغّل المنصة ويمكن أن تتغير في أي وقت دون إشعار. ولا يمكننا ضمان بقاء قناة أو مباراة أو فيلم أو مسلسل بعينه متاحًا طوال مدة اشتراكك.',
  'Household use only': 'للاستعمال المنزلي فقط',
  'Your credentials are for private use in a single household. You may not resell them, share them publicly, use them for commercial or public screening, or redistribute the stream. If we find credentials being used this way, we may close the account without a refund.':
    'بيانات الدخول مخصّصة للاستعمال الخاص داخل منزل واحد. ولا يجوز لك إعادة بيعها أو مشاركتها علنًا أو استعمالها للعرض التجاري أو العمومي أو إعادة توزيع البث. وإذا وجدنا استعمالًا من هذا النوع، جاز لنا إغلاق الحساب دون استرجاع.',
  'Your responsibilities': 'مسؤولياتك',
  'Give accurate contact details so we can deliver your credentials.':
    'تقديم بيانات تواصل صحيحة حتى نتمكن من تسليم بيانات الدخول.',
  'Provide your own internet connection and a compatible device. Roughly 5 Mbps is needed for SD, 15 Mbps for HD and 25 Mbps for 4K.':
    'توفير اتصال إنترنت خاص بك وجهاز متوافق. يلزم نحو 5 ميغابت/ث لجودة SD، و15 ميغابت/ث لـ HD، و25 ميغابت/ث لـ 4K.',
  'Keep your credentials private.': 'الحفاظ على سرية بيانات الدخول.',
  'Comply with the laws that apply where you live, including any rules about which broadcast content you may lawfully access. You are responsible for that; we cannot advise on it.':
    'الالتزام بالقوانين السارية في مكان إقامتك، بما فيها القواعد المتعلقة بالمحتوى المُذاع الذي يحق لك الوصول إليه قانونًا. وهذه مسؤوليتك أنت؛ ولا يمكننا تقديم المشورة بشأنها.',
  'Delivery': 'التسليم',
  'We aim to send credentials within ten minutes during support hours. Orders placed overnight are handled the following morning. Delivery is by WhatsApp or email to the contact details you gave us.':
    'نسعى لإرسال بيانات الدخول خلال عشر دقائق في أوقات الدعم. والطلبات الليلية تُعالَج صباح اليوم التالي. ويتم التسليم عبر واتساب أو البريد الإلكتروني إلى البيانات التي زوّدتنا بها.',
  'Availability and interruptions': 'التوفّر والانقطاعات',
  "We target high availability but cannot promise an uninterrupted service. Outages on the platform operator's side, on your internet connection, or on your device are outside our control. Where a line is genuinely down for an extended period, we will replace it and extend your subscription by the time lost. That is the remedy we offer.":
    'نستهدف توفّرًا عاليًا لكننا لا نستطيع الوعد بخدمة بلا انقطاع. فالأعطال لدى مشغّل المنصة أو في اتصالك بالإنترنت أو في جهازك خارجة عن سيطرتنا. وحين يتوقف خط فعلًا لمدة طويلة، نستبدله ونمدّد اشتراكك بمقدار الوقت الضائع. وهذا هو الحل الذي نقدّمه.',
  'Free 24-hour test': 'تجربة مجانية 24 ساعة',
  'One free test per household, lasting 24 hours from activation. It is the full service so you can judge it before paying. We may decline a test if we believe the offer is being abused.':
    'تجربة مجانية واحدة لكل منزل، مدتها 24 ساعة من التفعيل. وهي الخدمة كاملة حتى تحكم عليها قبل الدفع. ويجوز لنا رفض التجربة إذا رأينا أن العرض يُساء استعماله.',
  'Ending a subscription': 'إنهاء الاشتراك',
  'Subscriptions do not renew automatically. Yours simply ends on its expiry date unless you message us to extend it. You can stop using the service at any time; see the refund policy below for what that means financially.':
    'الاشتراكات لا تتجدّد تلقائيًا. فاشتراكك ينتهي ببساطة في تاريخ انتهائه ما لم تراسلنا لتمديده. ويمكنك التوقف عن استعمال الخدمة في أي وقت؛ وانظر سياسة الاسترجاع أدناه لمعرفة الأثر المالي لذلك.',
  'Our liability': 'مسؤوليتنا',
  'Our liability to you is limited to the amount you paid for your current subscription period. We are not liable for indirect losses. Nothing here limits liability that cannot lawfully be limited.':
    'تقتصر مسؤوليتنا تجاهك على المبلغ الذي دفعته عن فترة اشتراكك الحالية. ولا نتحمل مسؤولية الخسائر غير المباشرة. ولا شيء هنا يحدّ من مسؤولية لا يجوز قانونًا الحدّ منها.',
  'Changes to these terms': 'تعديل هذه الشروط',
  'We may update these terms. The version on this page at the time of your order is the one that applies to that order.':
    'يجوز لنا تحديث هذه الشروط. والنسخة الموجودة في هذه الصفحة وقت طلبك هي التي تسري على ذلك الطلب.',
  'Law': 'القانون المطبّق',
  'These terms are governed by Moroccan law. If you are a consumer resident in the EU or UK, this does not deprive you of the protection of mandatory consumer law in your country of residence, and you keep the right to bring proceedings there.':
    'تخضع هذه الشروط للقانون المغربي. وإذا كنت مستهلكًا مقيمًا في الاتحاد الأوروبي أو المملكة المتحدة، فإن ذلك لا يحرمك من حماية القواعد الآمرة لحماية المستهلك في بلد إقامتك، وتحتفظ بحق التقاضي هناك.',

  'In short': 'باختصار',
  'Once your credentials have been sent, the subscription is not refundable.':
    'بمجرد إرسال بيانات الدخول، لا يكون الاشتراك قابلًا للاسترجاع.',
  'The free 24-hour test exists so you can try the full service on your own device before you pay anything.':
    'التجربة المجانية لمدة 24 ساعة موجودة تحديدًا لتجرّب الخدمة كاملة على جهازك قبل أن تدفع أي شيء.',
  'Why': 'لماذا',
  'A subscription is digital content delivered immediately. Once credentials are issued the access has been granted and cannot be returned.':
    'الاشتراك محتوى رقمي يُسلَّم فورًا. وبمجرد إصدار بيانات الدخول يكون الوصول قد مُنح ولا يمكن إعادته.',
  'Your EU right of withdrawal, and how it applies': 'حقك في التراجع داخل الاتحاد الأوروبي وكيف يسري',
  'If you are a consumer in the EU or UK, you normally have 14 days to withdraw from an online purchase. For digital content delivered immediately, that right is lost once delivery begins — but only if you asked for immediate delivery and acknowledged that you would lose the right.':
    'إذا كنت مستهلكًا في الاتحاد الأوروبي أو المملكة المتحدة، فلك عادةً 14 يومًا للتراجع عن شراء إلكتروني. وفي المحتوى الرقمي المسلَّم فورًا يسقط هذا الحق بمجرد بدء التسليم — لكن فقط إذا طلبت التسليم الفوري وأقررت بأنك ستفقد هذا الحق.',
  'That is exactly what the confirmation box on our order form does. By ticking it you ask us to supply your credentials straight away and accept that your 14-day withdrawal right ends when we send them.':
    'وهذا بالضبط ما يفعله مربع التأكيد في نموذج الطلب لدينا. فبوضع العلامة فيه تطلب منا تزويدك ببيانات الدخول فورًا وتقبل أن حقك في التراجع خلال 14 يومًا ينتهي عند إرسالها.',
  'If you have not yet received your credentials, the withdrawal right still stands.':
    'وإذا لم تكن قد استلمت بيانات الدخول بعد، فحق التراجع ما يزال قائمًا.',
  'Message us before delivery and we will refund you in full.':
    'راسلنا قبل التسليم وسنعيد لك المبلغ كاملًا.',
  'When we do refund': 'متى نُرجع المبلغ',
  'You paid but we never delivered your credentials.':
    'دفعت ولم نسلّمك بيانات الدخول إطلاقًا.',
  'You were charged twice for the same subscription.':
    'خُصم منك مرتين مقابل الاشتراك نفسه.',
  'We are unable to provide the service at all for a sustained period and cannot replace your line.':
    'عجزنا تمامًا عن تقديم الخدمة لفترة ممتدة ولم نستطع استبدال خطك.',
  'In those cases we refund to the original payment method. Allow up to ten working days for it to appear.':
    'في هذه الحالات نُرجع المبلغ إلى وسيلة الدفع الأصلية. وقد يستغرق ظهوره حتى عشرة أيام عمل.',
  'When we do not refund': 'متى لا نُرجع المبلغ',
  'You changed your mind after receiving working credentials.':
    'غيّرت رأيك بعد استلام بيانات دخول تعمل.',
  'A specific channel or event you wanted is not carried, or was removed by the platform.':
    'قناة أو حدث بعينه أردته غير متوفر، أو أزالته المنصة.',
  'Your internet connection or device cannot handle the stream.':
    'اتصالك بالإنترنت أو جهازك لا يتحمّل البث.',
  'Your account was closed for breaching the household-use rule.':
    'أُغلق حسابك لمخالفة قاعدة الاستعمال المنزلي.',
  'Problems before you ask for money back': 'المشاكل قبل طلب استرجاع المال',
  'Most faults are fixable in minutes. Message us on WhatsApp first — replacing a line, resetting credentials or reconfiguring an app resolves the large majority of issues, and we add any lost time to your subscription.':
    'أغلب الأعطال تُصلَح في دقائق. راسلنا على واتساب أولًا — فاستبدال خط أو إعادة ضبط بيانات الدخول أو إعادة إعداد التطبيق يحل الغالبية العظمى من المشاكل، ونضيف أي وقت ضائع إلى اشتراكك.',

  'Who controls your data': 'من يتحكم في بياناتك',
  'Swift Channels. Contact:': 'سويفت تشانلز. للتواصل:',
  'Worth knowing': 'من المفيد معرفته',
  'This website has no database, no accounts and no tracking. The order form runs entirely in your browser and builds a message for you to send us. Nothing is submitted to a server here.':
    'هذا الموقع بلا قاعدة بيانات وبلا حسابات وبلا تتبّع. ونموذج الطلب يعمل بالكامل داخل متصفحك ويكوّن رسالة ترسلها أنت إلينا. ولا يُرسَل شيء إلى أي خادم هنا.',
  'What we collect and why': 'ما الذي نجمعه ولماذا',
  'What you send us': 'ما ترسله إلينا',
  '— your name, WhatsApp number, email if you give one, your device and any MAC address, and your order details. We need these to create your account and deliver your credentials. Legal basis: performance of a contract.':
    '— اسمك ورقم واتساب وبريدك الإلكتروني إن قدّمته وجهازك وعنوان MAC إن وُجد وتفاصيل طلبك. ونحتاج هذه البيانات لإنشاء حسابك وتسليم بيانات الدخول. الأساس القانوني: تنفيذ عقد.',
  'Payment records': 'سجلات الدفع',
  '— we see the name, amount and date associated with your payment, and the receipt you send us. We never see your card number. Legal basis: contract and legal record-keeping.':
    '— نرى الاسم والمبلغ والتاريخ المرتبطة بدفعتك، والإيصال الذي ترسله إلينا. ولا نرى رقم بطاقتك أبدًا. الأساس القانوني: العقد وحفظ السجلات القانونية.',
  'Your messages': 'رسائلك',
  '— the WhatsApp or email conversation, kept so we can support you and handle renewals or disputes.':
    '— محادثة واتساب أو البريد، تُحفَظ لنتمكن من دعمك ومعالجة التجديدات أو النزاعات.',
  'What we do not collect': 'ما لا نجمعه',
  'No cookies, analytics, advertising pixels or visitor tracking on this site.':
    'لا ملفات تعريف ارتباط ولا تحليلات ولا بكسلات إعلانية ولا تتبّع للزوار في هذا الموقع.',
  'No card or bank details — payments are handled entirely by PayPal.':
    'لا بيانات بطاقات أو بنوك — المدفوعات يعالجها باي بال بالكامل.',
  'No record of what you watch. We do not have access to your viewing activity.':
    'لا سجل لما تشاهده. ولا نملك أي وصول إلى نشاط مشاهدتك.',
  'Local storage': 'التخزين المحلي',
  'When you click through to pay, your order details are saved in your own browser so the confirmation page can show them back to you. This stays on your device, is never transmitted to us, and you can clear it through your browser at any time.':
    'عند انتقالك إلى الدفع تُحفَظ تفاصيل طلبك داخل متصفحك أنت لتتمكن صفحة التأكيد من عرضها عليك. وتبقى هذه البيانات على جهازك ولا تُرسَل إلينا أبدًا، ويمكنك مسحها من متصفحك في أي وقت.',
  'Who we share it with': 'مع من نتشاركها',
  'The platform operator': 'مشغّل المنصة',
  '— the minimum needed to create and maintain your account.':
    '— الحد الأدنى اللازم لإنشاء حسابك والحفاظ عليه.',
  'Payment providers': 'مزوّدو الدفع',
  '— PayPal, which processes your payment under its own privacy policy.':
    '— باي بال، الذي يعالج دفعتك وفق سياسة الخصوصية الخاصة به.',
  "— as the channel we communicate through, under Meta's policies.":
    '— بوصفه قناة التواصل بيننا، وفق سياسات ميتا.',
  'We do not sell your data or share it for marketing.':
    'نحن لا نبيع بياناتك ولا نتشاركها لأغراض تسويقية.',
  'International transfers': 'النقل الدولي للبيانات',
  'We are based in Morocco, so if you are in the EU or UK your data is processed outside your country. We keep transfers to the minimum needed to run your subscription.':
    'مقرّنا في المغرب، لذا إذا كنت في الاتحاد الأوروبي أو المملكة المتحدة فإن بياناتك تُعالَج خارج بلدك. ونُبقي عمليات النقل عند الحد الأدنى اللازم لتشغيل اشتراكك.',
  'How long we keep it': 'مدة الاحتفاظ بها',
  'Order and payment records for three years after your subscription ends, so we can handle disputes and meet record-keeping obligations. Message threads are kept while you are a customer and for a reasonable period afterwards.':
    'سجلات الطلب والدفع لمدة ثلاث سنوات بعد انتهاء اشتراكك، لنتمكن من معالجة النزاعات والوفاء بالتزامات حفظ السجلات. أما المحادثات فتُحفَظ ما دمت زبونًا ولمدة معقولة بعد ذلك.',
  'Your rights': 'حقوقك',
  'If you are in the EU or UK you can ask us to give you a copy of your data, correct it, delete it, restrict how we use it, or object to our using it. Email':
    'إذا كنت في الاتحاد الأوروبي أو المملكة المتحدة، يمكنك أن تطلب منا نسخة من بياناتك أو تصحيحها أو حذفها أو تقييد استعمالنا لها أو الاعتراض عليه. راسلنا على',
  'and we will respond within 30 days. If you are unhappy with our response you can complain to the data protection authority in your country.':
    'وسنرد خلال 30 يومًا. وإذا لم يُرضك ردّنا، يمكنك تقديم شكوى إلى سلطة حماية البيانات في بلدك.',
  'Children': 'القاصرون',
  'This service is not intended for anyone under 18 and we do not knowingly collect data from children.':
    'هذه الخدمة ليست موجّهة لمن هم دون 18 سنة، ولا نجمع عن علم أي بيانات تخص الأطفال.',
  'Changes': 'التعديلات',
  'If we change this notice we will update the date at the top of this page.':
    'إذا عدّلنا هذا الإشعار فسنحدّث التاريخ في أعلى هذه الصفحة.'
}

  };

  /* merge into the main table loaded by assets/i18n.js */
  if(window.I18N){
    Object.keys(EXTRA).forEach(lang => {
      window.I18N[lang] = window.I18N[lang] || {};
      Object.assign(window.I18N[lang], EXTRA[lang]);
    });
  }
})();
